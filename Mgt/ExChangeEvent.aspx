<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/Mgt.master" AutoEventWireup="true" CodeFile="ExChangeEvent.aspx.cs" Inherits="Mgt_ExChangeEvent" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    <script type="text/javascript">
        ...
        function OpenWindows() {
            var open = window.open('AutoSendMail.aspx?Ex=1', '', 'width=1000,height=600');
        }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div class="path txtS mb20">現在位置：<a href="#">繼續教育課程管理</a> <i class="fa fa-angle-right"></i><a href="ExChangeEvent.aspx">繼續教育報名管理</a></div>
    ...
    <div class="both mb20">
        <fieldset>
            ...
            <div class="left w8">
                ...
                <%--課程分類<asp:DropDownList ID="ddl_Class2" runat="server" DataTextField="Mval" DataValueField="PVal"></asp:DropDownList>--%>
            ...
            <div class="right">
                ...
                <% if (userInfo.AdminIsInsert == true)
                    { %>
                <input name="btnInsert" type="button" value="新增" onclick="location.href = 'Event_AE.aspx?Work=N&&Exchange=1'" />
       ...
       <%--     <asp:TemplateField HeaderText="名單" ItemStyle-HorizontalAlign="Center">
            ...
            <asp:TemplateField HeaderText="審核" ItemStyle-HorizontalAlign="Center">
                <ItemTemplate>
                    ...
                    <%# ( Convert.ToDateTime(Eval("StartTime")) <= DateTime.Today                          
                        ...
                        "<a href='./EventAudit.aspx?a=" +Request.QueryString["a"] + "&n=NN&sno="+Eval("EventSNO").ToString()+" &Exchange=1'><i class='fa fa-pen-square' aria-hidden='true'></i></a>"                  
                        : "尚未開放"
                    %>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Center"></ItemStyle>
            ...
            <asp:TemplateField HeaderText="修改" ItemStyle-HorizontalAlign="Center">
                <ItemTemplate>
                    ...
                    <a href='./Event_AE.aspx?a=<%=Request.QueryString["a"] %>&n=NN&&Exchange=1&sno=<%#Eval("EventSNO").ToString() %>&csno=<%#Eval("id").ToString() %>'><i class="fa fa-pen-square"></i></a>
                    <% } else { %>
                         無權限
                    <% } %>
                </ItemTemplate>
            ...
            <asp:TemplateField HeaderText="刪除" ItemStyle-HorizontalAlign="Center">
             ...
             <%--<asp:TemplateField HeaderText="通知" ItemStyle-HorizontalAlign="Center">
                <ItemTemplate>
                    <% if (userInfo.AdminIsUpdate == true) { %>
                    <a href="#"  onclick="var winvar = window.open('./SendMail.aspx?sno=<%# Eval("EventSNO") %>&ExChange=1','winname','width=800 height=550 location=no,menubar=no status=no,toolbar=no');"><i class="fa fa-pen-square"></i></a>
                    <% } else { %>
                         無權限
                    <% } %>
                </ItemTemplate>
