...
public partial class Mgt_ExChangeScoreUpload : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            lb_Notice.Text = @"上傳方式:" + "<br>" + @"1.請先選好要上傳時數的「繼續教育規劃類別」，再點擊右側「下載格式」按鈕下載格式檔。" + "<br>" + @" 2.將資料依照格式檔欄位及格式輸入後存檔，點擊右側「上傳」按鈕選擇存好檔的檔案進行上傳，上傳後，請務必使用「繼續教育課程時數上傳紀錄查詢」確認上傳結果。";
        }
    }
                ...
                for (int rowIterator = 2; rowIterator <= noOfRow; rowIterator++)
                {
                    ...
                    if (ddl_UploadType.SelectedValue == "2")
                    {
                        ...
                        try
                        {
                            ...
                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 3].Text)) item.Status += "課程名稱空白! ;"; item.style = "color:red; font-weight:bold;";
                            item.style = "color:red; font-weight:bold;";
                            item.CourseName = workSheet.Cells[rowIterator, 3].Text;
                            string[] format = { "yyyy/MM/dd" };

                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 4].Text)) { item.Status += "日期空白! ;"; item.style = "color:red; font-weight:bold;"; } else
                            {
                            ...
                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 5].Text)) { item.Status += "學分空白! ;"; item.style = "color:red; font-weight:bold;"; } else
                            {
                            ...
                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 6].Text)) item.Status += "上課地點空白! ;"; item.style = "color:red; font-weight:bold;";
                            item.CLocation = workSheet.Cells[rowIterator, 6].Text; 
                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 7].Text)) item.Status += "辦理單位空白! ;"; item.style = "color:red; font-weight:bold;";
                            item.Uni = workSheet.Cells[rowIterator, 7].Text;
                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 8].Text)) item.Status += "類別空白! ;"; item.style = "color:red; font-weight:bold;";
