...
public partial class Mgt_ScoreUpload : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            lb_Notice.Text = @"上傳方式:" + "<br>" + @"1.請先選好要上傳時數的「課程規劃類別」和「欲上傳的課程」，再點擊右側「下載格式」按鈕下載格式檔。" + "<br>"+ @" 2.將資料依照格式檔欄位及格式輸入後存檔，點擊右側「上傳」按鈕選擇存好檔的檔案進行上傳，上傳後，請務必使用「新訓課程時數上傳紀錄查詢」確認上傳結果。";
        }
    }
                ...
                for (int rowIterator = 2; rowIterator <= noOfRow; rowIterator++)
                {
                    if (ddl_UploadType.SelectedValue == "1")
                    {
                        ...
                        try
                        {
                            ...
                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 1].Text)) item.Status += "姓名空白! ;"; item.style = "color:red; font-weight:bold;";
                            item.MEMBER_NAME = workSheet.Cells[rowIterator, 1].Text;
                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 2].Text)) item.Status += "身分證空白! ;"; item.style = "color:red; font-weight:bold;";
                            item.ID = workSheet.Cells[rowIterator, 2].Text.Trim();

                            int score;
                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 3].Text)) { item.Status += "分數空白! ;"; item.style = "color:red; font-weight:bold;"; } else
                            {
                                var isScore = Int32.TryParse(workSheet.Cells[rowIterator, 3].Text, out score);
                                item.Score = score.ToString();
                                if (!isScore) item.Status += "分數格式不正確! ;"; item.style = "color:red; font-weight:bold;";
                            }

                            string[] format = { "yyyy/M/dd", "yyyy/MM/dd" };

                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 4].Text)) { item.Status += "日期空白! ;"; item.style = "color:red; font-weight:bold;"; }
                            else
                            {
...
DateTimeStyles.None, out dateTime);
                            ...
                            if (!isPassScore) item.Status += "分數格式不正確! ;"; item.style = "color:red; font-weight:bold;";
                        ...
                        {
                            if (!string.IsNullOrEmpty(item.ID) && item.Status != "身分證空白! ;上課地點空白! ;授課類別空白! ;上課日期空白! ;辦理單位空白! ;課程代號空白! ;")
                            {
                                rowid += 1;
                                orderItem.Add(item);
                            }
                    ...
                    if (ddl_UploadType.SelectedValue == "0")
                    {
                        ...
                        try
                        {
                            ...
                            {
                                item.Status += "身分證空白! ;"; item.style = "color:red; font-weight:bold;";
                            }
                            else if (!ValidIsID(workSheet.Cells[rowIterator, 2].Text))
                            {
                                ...
                                {
                                    item.Status += "身分證/居留證格式有誤！;"; item.style = "color:red; font-weight:bold;";
                                }
                            }
                            item.ID = workSheet.Cells[rowIterator, 2].Text;
                            //int score;
                            ...
                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 3].Text)) item.Status += "上課地點空白! ;"; item.style = "color:red; font-weight:bold;";
                            item.ClassLocation = workSheet.Cells[rowIterator, 3].Text;
                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 7].Text)) item.Status += "授課類別空白! ;"; item.style = "color:red; font-weight:bold;";
                            item.Ctype = workSheet.Cells[rowIterator, 7].Text;
                            DateTime date;
                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 4].Text))
                            {
                                item.Status += "上課日期空白! ;"; item.style = "color:red; font-weight:bold;";
                            }
                            else {
                                ...
                                {
                                    item.Date = workSheet.Cells[rowIterator, 4].Text;
                                    item.Status += "日期格式不正確! ;"; item.style = "color:red; font-weight:bold;";
                                }
                                else
                                {
                            ...
                            if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 5].Text)) item.Status += "辦理單位空白! ;"; item.style = "color:red; font-weight:bold;";
                            item.Unit = workSheet.Cells[rowIterator, 5].Text;

                            if (ddl_CourseName.SelectedValue == "998" || ddl_CourseName.SelectedValue == "999")
                            {
                                if (string.IsNullOrEmpty(workSheet.Cells[rowIterator, 6].Text)) { item.Status += "課程代號空白! ;"; item.style = "color:red; font-weight:bold;"; } else
                                {
                                    ...
                                    for (int i = 0; i < CourseSNOArray.Length; i++)
                                    {
                                        ...
                                        if (!CourseSNO)
                                        {
                                            item.Status += "課程代號不正確! ;"; item.style = "color:red; font-weight:bold;";
                                        }
                                        else if (Utility.CheckCourseSNO(CourseSNOArray[i]) == false)
                                        {
                                            item.Status += "無此課程代號! ;"; item.style = "color:red; font-weight:bold;";
                                        }
                                        else
                                        {
                        ...
                        {
                            if (item.Status != "身分證空白! ;上課地點空白! ;授課類別空白! ;上課日期空白! ;辦理單位空白! ;課程代號空白! ;")
                            {
                                rowid += 1;
                                orderItem.Add(item);
                            }
